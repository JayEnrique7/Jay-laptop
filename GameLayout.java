        ...
        for (int i = 0 ; i < 9; i++) {
            ...
            boardButton[i].addActionListener(new ActionListener()  {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    int currPlayer = BoardRules.getInstance().getcurrPlayer();
                    try {
                        BoardRules.getInstance().checkMove(tempIndx, field);
                        boardField[tempIndx].setMove(player[currPlayer].getCharacter());
                        if (BoardRules.getInstance().Check(field, player[currPlayer])) {
                            updateText(textArea, player[0], player[1], player[currPlayer]);
                        } else {
                            updateText(textArea, player[0], player[1], player[currPlayer]);
                        }
                    } catch (Exception m) {
